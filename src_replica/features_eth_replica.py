...
def build_protocol_window_features(packet_df: pd.DataFrame, window_size: int, max_windows: Optional[int]) -> pd.DataFrame:
    ...
    for w in range(n_windows):
        ...
        # ── Phase 1 DPI: payload entropy aggregates for this window ─────────
        if "payload_entropy" in packet_df.columns:
            ent_win = pd.to_numeric(packet_df["payload_entropy"].iloc[s:e], errors="coerce").fillna(0.0)
            payload_entropy_mean = float(ent_win.mean())
            payload_entropy_std = float(ent_win.std(ddof=0)) if len(ent_win) > 1 else 0.0
            payload_entropy_max = float(ent_win.max())
        else:
            payload_entropy_mean = payload_entropy_std = payload_entropy_max = 0.0

        ip_proto_mean = 0.0
        dst_port_mean = 0.0
        if "ip_proto" in packet_df.columns:
            ip_proto_mean = float(pd.to_numeric(packet_df["ip_proto"].iloc[s:e], errors="coerce").fillna(0.0).mean())
        if "dst_port" in packet_df.columns:
            dst_port_mean = float(pd.to_numeric(packet_df["dst_port"].iloc[s:e], errors="coerce").fillna(0.0).mean())

        # Payload byte means over this window (16 values)
        payload_byte_means: Dict[str, float] = {}
        for col in STANDARD_ETH_PAYLOAD_COLS:
            if col in packet_df.columns:
                payload_byte_means[col] = float(
                    pd.to_numeric(packet_df[col].iloc[s:e], errors="coerce").fillna(0.0).mean()
                )
            else:
                payload_byte_means[col] = 0.0
        ...
        rows.append({
            ...
            'ip_proto_mean': ip_proto_mean,
            'dst_port_mean': dst_port_mean,
            **payload_byte_means,
        })
...
def build_image_window_features(npy_path: str, window_size: int, max_windows: Optional[int]) -> pd.DataFrame:
    ...
    for w in range(n_windows):
        ...
        gx = np.abs(np.diff(win, axis=2)).mean() if win.shape[2] > 1 else 0.0
        gy = np.abs(np.diff(win, axis=1)).mean() if win.shape[1] > 1 else 0.0
        gt = np.abs(np.diff(win, axis=0)).mean() if win.shape[0] > 1 else 0.0
        rows.append({
            'window_idx': w,
            'i_mean': mean_val,
            'i_std': std_val,
